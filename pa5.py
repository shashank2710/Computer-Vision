...
class App:    
    ...
    def __init__(self, video_src, paused = False):
        ...
        self.trackers = []
        self.paused = paused
        self.cap = cv2.VideoCapture('example1.mov')
        ...
        while(1):
            ...
            if cnts != [] or k == 0:
                ...
                self.datastatic=np.vstack((self.datastatic,np.array(("Still",frame_count,frame_count,xs,ys,class_names[p],probs[index,p]))))
            frame_count += 1
            l = np.array(self.datastatic)
            k = l[1:, 6]
            z = heapq.nlargest(5,k)
            key = cv2.waitKey(1) & 0xFF
            if key == ord("q"):
                #print(str(self.datadynamic))
                for dat in self.datadynamic:
                	f1.write('|'.join(list(map(str,dat))) + '\n')
                for dat in self.datastatic:
                	f.write('|'.join(list(map(str,dat))) + '\n')
                index = heapq.nlargest(5,range(len(k)), k.__getitem__)
                for i in range (0,5):
                	ind=index[i]+1
                	value=l[ind,:]
                	f2.write('|'.join(list(map(str,value)))+'\n')
                len(save)
                break
